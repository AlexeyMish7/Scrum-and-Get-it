...
/**
 * COMPANY RESEARCH: POST /api/generate/company-research
 *
 * Body: { companyName: string, jobId?: number }
 * Headers: Authorization (JWT) or X-User-Id (dev mode)
 *
 * Response: 201 with { id, kind, created_at, preview, content, persisted, metadata }
 */
export async function handleCompanyResearch(
  req: IncomingMessage,
  res: ServerResponse,
  url: URL,
  reqId: string,
  ...
  // Rate limit (10/min per user - research is less intensive than generation)
