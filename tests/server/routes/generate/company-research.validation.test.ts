...
async function importFreshRoute() {
  vi.resetModules();
  return await import("@server/routes/generate/company-research");
}

describe("Generate Company Research Route - validation", () => {
  beforeEach(() => {
    rateLimiter.checkLimit.mockReset();
    http.readJson.mockReset();
    orchestrator.handleCompanyResearch.mockReset();
  });
  ...
  it("throws 429 rate_limited and sets Retry-After", async () => {
    ...
    await expect(
      route.post(
        {} as any,
        res,
        new URL("/api/generate/company-research", "http://localhost"),
        "req-1",
        "user-1",
  ...
  it("throws 400 bad_request when companyName is missing", async () => {
    const route = await importFreshRoute();
  ...
  it("throws 400 bad_request when jobId is present but not numeric", async () => {
    const route = await importFreshRoute();

    rateLimiter.checkLimit.mockReturnValue({ ok: true });
    http.readJson.mockResolvedValueOnce({ companyName: "Acme", jobId: "nope" });
  ...
  it("throws 502 ai_error when orchestrator returns error", async () => {
    ...
    rateLimiter.checkLimit.mockReturnValue({ ok: true });
    http.readJson.mockResolvedValueOnce({ companyName: "Acme" });
    orchestrator.handleCompanyResearch.mockResolvedValueOnce({
      error: "ai down",
    });
  ...
  it("returns 201 with persisted=false when Supabase env is missing", async () => {
    ...
    rateLimiter.checkLimit.mockReturnValue({ ok: true });
    http.readJson.mockResolvedValueOnce({ companyName: " Acme  ", jobId: 123 });
    orchestrator.handleCompanyResearch.mockResolvedValueOnce({
      artifact: {
        id: "a1",
        user_id: "user-1",
    ...
    expect(body.kind).toBe("company_research");
