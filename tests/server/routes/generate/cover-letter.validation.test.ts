...
async function importFreshRoute() {
  vi.resetModules();
  return await import("@server/routes/generate/cover-letter");
}

describe("Generate Cover Letter Route - validation", () => {
  beforeEach(() => {
    rateLimiter.checkLimit.mockReset();
    http.readJson.mockReset();
    orchestrator.handleGenerateCoverLetter.mockReset();
  });

  it("throws 429 rate_limited and sets Retry-After", async () => {
    const route = await importFreshRoute();

    rateLimiter.checkLimit.mockReturnValue({ ok: false, retryAfterSec: 34 });
    ...
    await expect(
      route.post(
        {} as any,
        res,
        new URL("/api/generate/cover-letter", "http://localhost"),
        "req-1",
        "user-1",
    ...
    expect(state.headers["Retry-After"]).toBe("34");
    expect(orchestrator.handleGenerateCoverLetter).not.toHaveBeenCalled();
  });
  ...
  it("throws 502 ai_error when orchestrator returns error", async () => {
    ...
    orchestrator.handleGenerateCoverLetter.mockResolvedValueOnce({
      error: "ai is down",
    });
