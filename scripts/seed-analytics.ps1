# Seed sample analytics data via the server ingest endpoint
# Usage: Open PowerShell at the repo root and run:
#   .\scripts\seed-analytics.ps1 -UserId aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaaa -Host http://localhost:8787
param(
  [Parameter(Mandatory=$true)]
  [string]$UserId,

  [string]$Host = "http://localhost:8787"
)
...
function Post-Payload($payload) {
  ...
  try {
    $res = Invoke-RestMethod -Uri ("$Host/api/analytics/ingest") -Method Post -ContentType 'application/json' -Body $json
...
# A small set of sample interviews across months and formats
$items = @()
