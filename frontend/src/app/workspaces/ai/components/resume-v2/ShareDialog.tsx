...
export default function ShareDialog({ open, onClose }: Props) {
  ...
  const [privacy, setPrivacy] = useState<ResumeShare["privacy"]>("link");
  const [expires, setExpires] = useState<string | "">("");
  const [link, setLink] = useState<string | null>(null);

  const handleCreate = () => {
    if (!draft) return;
    const s = createShare(draft.id, {
      permissions,
      privacy,
        ...
        <Stack spacing={2} sx={{ pt: 1 }}>
          <FormControl fullWidth>
            ...
            <Select
              ...
              label="Permissions"
              onChange={(e) =>
                setPermissions(e.target.value as ResumeShare["permissions"])
              }
            >
          ...
          <FormControl fullWidth>
            ...
            <Select
              ...
              label="Privacy"
              onChange={(e) =>
                setPrivacy(e.target.value as ResumeShare["privacy"])
              }
            >
          ...
          {link ? (
            ...
            <Typography variant="caption" color="text.secondary">
              Create a shareable link to allow reviewers to comment on this
              resume (stored locally for now).
            </Typography>
          )}
        </Stack>
      </DialogContent>
