...
export default function ResumeVersionsPanel({ open, onClose }: Props) {
  const { drafts, getActiveDraft, editSection } = useResumeDraftsV2();
  ...
  const versions = useMemo(() => {
    ...
    return versionsApi.listAllIncludingArchived(activeDraft.id).sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
  ...
  const handleCreate = () => {
    ...
    const v = versionsApi.createVersion(activeDraft.id, newName || activeDraft.name, content, {
  ...
  const handleApplyVersionToDraft = async (id: string) => {
    ...
    for (const s of sections) {
      try {
        // @ts-ignore - editSection accepts unknown
        await editSection(s, content[s]);
      } catch (e) {
  ...
  const mergeContents = (source: any, dest: any, preferSource = true) => {
    const out: any = {};
    const sections = ["summary", "skills", "experience", "education", "projects"];
    for (const s of sections) {
      ...
      const isEmpty = (v: any) => v === undefined || v === null || (Array.isArray(v) && v.length === 0) || (typeof v === 'string' && v.trim() === '');
      if (preferSource) {
        out[s] = !isEmpty(a) ? a : b;
      } else {
        out[s] = !isEmpty(b) ? b : a;
  ...
  const renderVersionSummary = (v: any) => (
    <div>
      <Typography variant="subtitle2">{v.name} {v.isDefault && <Chip size="small" icon={<StarIcon fontSize="small"/>} label="Default" />}</Typography>
      <Typography variant="caption" color="text.secondary">{v.description}</Typography>
      <Typography variant="caption" color="text.secondary" display="block">Created: {new Date(v.createdAt).toLocaleString()}</Typography>
    </div>
  );
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ pt: 1 }}>
          <Stack direction="row" spacing={1} alignItems="center">
            <Button startIcon={<SaveIcon />} onClick={() => setCreateOpen(true)}>
              Create Version
            </Button>
          ...
          <List>
            {versions.map((v) => (
              <ListItem key={v.id} secondaryAction={<Stack direction="row" spacing={1}>
                <Tooltip title="Compare"><IconButton onClick={()=>openCompare(v.id)}><CompareArrowsIcon /></IconButton></Tooltip>
                <Tooltip title="Set Default"><IconButton onClick={() => handleSetDefault(v.id)}><StarIcon /></IconButton></Tooltip>
                <Tooltip title="Apply to Draft"><IconButton onClick={() => handleApplyVersionToDraft(v.id)}><SaveIcon /></IconButton></Tooltip>
                <Tooltip title="Archive"><IconButton onClick={() => handleArchive(v.id)}><ArchiveIcon /></IconButton></Tooltip>
                <Tooltip title="Delete"><IconButton onClick={() => handleDelete(v.id)}><DeleteIcon /></IconButton></Tooltip>
              </Stack>}>
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ pt: 1 }}>
            <TextField label="Version Name" fullWidth value={newName} onChange={(e)=>setNewName(e.target.value)} />
            <TextField label="Description" fullWidth multiline rows={3} value={newDesc} onChange={(e)=>setNewDesc(e.target.value)} />

            <FormControl fullWidth>
              <InputLabel id="version-job-label">Link to Job (optional)</InputLabel>
              <Select labelId="version-job-label" value={newJobId} label="Link to Job (optional)" onChange={(e)=>setNewJobId(e.target.value as number | "")}> 
                <MenuItem value=""><em>None</em></MenuItem>
                {jobs.map(j=> <MenuItem key={j.id} value={j.id}>{j.title} @ {j.company}</MenuItem>)}
              </Select>
            </FormControl>
          ...
          <Stack direction="row" spacing={2} sx={{ pt: 1 }}>
            <Box sx={{ flex: 1, border: '1px solid #eee', p: 1 }}>
              ...
              <FormControl fullWidth sx={{ mb: 1 }}>
                <Select value={leftId || ''} onChange={(e)=>setLeftId(e.target.value as string)}>
              ...
              <Divider />
              <Box sx={{ mt: 1 }}>
                <Typography variant="caption">{leftVer ? leftVer.description : 'No version selected'}</Typography>
              </Box>
            </Box>
            <Box sx={{ flex: 1, border: '1px solid #eee', p: 1 }}>
              <Typography variant="subtitle2">Right</Typography>
              <FormControl fullWidth sx={{ mb: 1 }}>
                <Select value={rightId || ''} onChange={(e)=>setRightId(e.target.value as string)}>
                  <MenuItem value=""><em>Choose</em></MenuItem>
                  {versions.map(v=> <MenuItem key={v.id} value={v.id}>{v.name}</MenuItem>)}
                </Select>
              </FormControl>
              <Divider />
              <Box sx={{ mt: 1 }}>
                <Typography variant="caption">{rightVer ? rightVer.description : 'No version selected'}</Typography>
              </Box>
            </Box>
          </Stack>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            ...
            onClick={async ()=>{
              ...
              const merged = mergeContents(left.content, right.content, /*preferSource=*/ true);
              const name = `Merge: ${left.name} → ${right.name}`;
              const v = versionsApi.createVersion(left.draftId, name, merged, { description: `Merged ${left.name} into ${right.name}` });
          ...
          <Button
            ...
            onClick={async ()=>{
              ...
              const right = rightId ? versionsApi.getVersion(rightId) : null;
              if (!left || !right) return;
              const merged = mergeContents(right.content, left.content, /*preferSource=*/ true);
              const name = `Merge: ${right.name} → ${left.name}`;
              const v = versionsApi.createVersion(left.draftId, name, merged, { description: `Merged ${right.name} into ${left.name}` });
              alert(`Created merged version: ${v.name}`);
              setCompareOpen(false);
            }}
          >
          ...
          <Button
            ...
            onClick={async ()=>{
              ...
              const merged = mergeContents(left ? left.content : {}, right ? right.content : {}, /*preferSource=*/ !!left);
              const sections = ["summary","skills","experience","education","projects"];
              for (const s of sections) {
                try {
                  // @ts-ignore
                  await editSection(s, merged[s]);
                } catch (e) {
      ...
      <Dialog open={Boolean(mergeSourceId)} onClose={()=>setMergeSourceId(null)}>
        <DialogTitle>Merge Version into Current Draft</DialogTitle>
        <DialogContent>
          <Stack spacing={2} sx={{ pt: 1 }}>
            <FormControl fullWidth>
              <InputLabel id="merge-version-label">Select version to merge</InputLabel>
              <Select labelId="merge-version-label" value={mergeSourceId||''} onChange={(e)=>setMergeSourceId(e.target.value as string)}>
                <MenuItem value=""><em>Choose</em></MenuItem>
                {versions.map(v=> <MenuItem key={v.id} value={v.id}>{v.name}</MenuItem>)}
              </Select>
            </FormControl>
            <Typography variant="caption">Merging will apply non-empty fields from the selected version into the current draft.</Typography>
          </Stack>
        </DialogContent>
        <DialogActions>
