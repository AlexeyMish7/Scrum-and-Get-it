...
export default function PipelinePage() {
  ...
  // Phase 2: New modal/drawer states
  ...
  // Keyboard shortcuts: A = add job, D = documents, M = analytics
  useEffect(() => {
  ...
  function applyFilters(rows: JobRow[] | undefined, filters?: JobFilters) {
    ...
    const filtered = source.filter((r) => {
      // Exclude archived jobs from the active pipeline view unless showArchived is true
      const isArchived =
      ...
      <Box sx={{ maxWidth: 1400, mx: "auto" }}>
        ...
        <Stack direction="row" spacing={1} sx={{ mb: 2 }}>
          ...
          <Button
            ...
            size="small"
          >
            {analyticsExpanded ? "Hide" : "Show"} Analytics (M)
          </Button>
        </Stack>
        ...
        <JobFormDialog
          ...
          onSuccess={(newJob) => {
            // Refresh the pipeline after adding a job
            setAddJobOpen(false);
            loadJobs();
            showSuccess(`Job "${newJob.job_title}" added successfully!`);
          }}
        />
