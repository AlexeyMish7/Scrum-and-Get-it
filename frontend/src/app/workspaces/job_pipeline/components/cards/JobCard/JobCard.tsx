...
type Props = {
  ...
  showMatchScore?: boolean; // Optional: enable match score display
};
...
const JobCard = memo(function JobCard({
  ...
  // Fetch match score if enabled and user is authenticated
  const jobId = job.id ? Number(job.id) : null;
  const { data: matchData, loading: matchLoading } = useJobMatch(
    showMatchScore ? user?.id : undefined,
    showMatchScore ? jobId : null
  );
  ...
  return (
    ...
    >
      <CardContent sx={{ p: 1 }}>
        ...
        {showMatchScore && matchLoading && (
          <Box sx={{ mt: 1 }}>
            <MatchScoreBadge score={0} loading={true} size="small" />
          </Box>
        )}
