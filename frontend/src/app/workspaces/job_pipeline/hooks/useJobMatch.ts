...
export function useJobMatch(
  ...
  const fetchMatch = useCallback(async () => {
    ...
    try {
      // Step 1: Check Supabase cache first (unless profile changed)
      if (!hasProfileChanged) {
        ...
        console.log(
          `[useJobMatch] Profile changed - regenerating analytics for job ${jobId}`
        );
      }

      console.log(
        `[useJobMatch] ${
          hasProfileChanged ? "Profile changed" : "Cache MISS"
        } for job ${jobId}, running AI analysis...`
      );
      ...
      const result: MatchData = {
        ...
        recommendations: [
          ...skillsMatch.missing
            .slice(0, 3)
            .map((skill) => `Consider learning ${skill} to improve your match`),
      ...
      // Reset profile change flag since we just regenerated analytics
      if (hasProfileChanged) {
        resetProfileChanged();
      }
  ...
  }, [userId, jobId, hasProfileChanged, resetProfileChanged]);
