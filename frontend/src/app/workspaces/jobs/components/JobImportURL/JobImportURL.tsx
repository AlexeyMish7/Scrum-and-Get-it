...
type ImportPayload = Record<string, any>;
...
export default function JobImportURL({ onImport }: Props) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const parsed = (resp?.data?.data ?? resp?.data ?? {}) as Record<string, any>;
  // raw parsed object received from the Edge Function

      const jobTitle = String(parsed.job_title ?? parsed.title ?? parsed.position ?? "").trim();
      const company = String(parsed.company_name ?? parsed.company ?? parsed.employer ?? "").trim();
      const rawDescription = String(parsed.job_description ?? parsed.description ?? "").trim();
      ...
      const description = rawDescription.slice(0, 2000);
          ...
          const parseSalary = (s: string | undefined) => {
            ...
            const re = /[$£€]?\s*([0-9]{1,3}(?:[,0-9]{0,})?(?:\.\d+)?)(?:\s*(k|K))?/g;
            const nums: number[] = [];
          ...
          const parseDeadline = (s: string | undefined) => {
            ...
            const monthNameRe = /\b(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:t(?:ember)?)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\b[^\n,]{0,30}\d{1,2},?\s*\d{4}/i;
      ...
      if (parsed.location && typeof parsed.location === "string") {
        const parts = parsed.location.split(",").map((s: string) => s.trim()).filter(Boolean);
        if (parts.length >= 2) {
        ...
        locationCity = parsed.location.city ?? parsed.location_city ?? locationCity;
        locationState = parsed.location.state ?? parsed.location_state ?? locationState;
      }

      // Build final normalized payload to pass to parent onImport.
      const finalData: Record<string, any> = { ...(parsed || {}) };
      ...
      try {
        const rawDeadline = String(finalData.deadline ?? parsed.deadline ?? "").trim();
      ...
      finalData.location = finalData.location ?? { city: finalData.location_city, state: finalData.location_state };
  ...
  // normalized final data ready for the form
      ...
      const returnedKeys = Object.keys(finalData).filter((k) => finalData[k] !== undefined && finalData[k] !== null && String(finalData[k]).toString().trim() !== "");

      const hasTitle = Boolean(finalData.job_title && String(finalData.job_title).trim());
      const hasCompany = Boolean(finalData.company_name && String(finalData.company_name).trim());
      const hasDescription = Boolean(finalData.job_description && String(finalData.job_description).trim());

      if (!hasTitle && !hasCompany && !hasDescription) {
        const msg = "Could not extract job data from this URL (function returned empty).";
        setImportResult({ status: "failed", fields: { title: false, company: false, description: false }, message: msg });
        handleError(msg);
      } else {
        onImport(finalData);
        const all = hasTitle && hasCompany && hasDescription;
        const status = all ? "success" : "partial";
        setImportResult({ status: status as any, fields: { title: hasTitle, company: hasCompany, description: hasDescription } });

        let msg = "Imported job data";
        if (returnedKeys.length > 0) msg += ` (fields: ${returnedKeys.join(", ")})`;
        if (truncated) msg += ". Description truncated to 2000 chars.";
        showSuccess(msg);
      }
    } catch (err: any) {
      setImportResult({ status: "failed", fields: { title: false, company: false, description: false }, message: String(err?.message ?? "Import failed") });
  ...
  return (
    <Box sx={{ display: "flex", gap: 1, alignItems: "center", mb: 2 }}>
      ...
      {importResult && (
        <Box sx={{ ml: 2, display: "flex", gap: 2, alignItems: "center" }} aria-live="polite">
          <Box sx={{ fontSize: 13, color: importResult.status === "failed" ? "error.main" : importResult.status === "success" ? "success.main" : "text.primary" }}>
            {importResult.status === "success" ? "Import: success" : importResult.status === "partial" ? "Import: partial" : "Import: failed"}
          </Box>
          <Box sx={{ fontSize: 13, color: importResult.fields.title ? "success.main" : "text.secondary" }}>Title {importResult.fields.title ? "✓" : "✕"}</Box>
          <Box sx={{ fontSize: 13, color: importResult.fields.company ? "success.main" : "text.secondary" }}>Company {importResult.fields.company ? "✓" : "✕"}</Box>
          <Box sx={{ fontSize: 13, color: importResult.fields.description ? "success.main" : "text.secondary" }}>Description {importResult.fields.description ? "✓" : "✕"}</Box>
        </Box>
      )}
    </Box>
  );
