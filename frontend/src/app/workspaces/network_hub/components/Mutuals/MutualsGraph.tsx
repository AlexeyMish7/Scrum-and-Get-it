...
export default function MutualsGraph({
  contactId,
}: {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (
          contactId &&
          !list.find((c) => String(c.id) === String(contactId))
        ) {
          const single = await qc.ensureQueryData({
  ...
  if (!contactId) {
    return (
      <Typography color="textSecondary">
        Select a contact to see mutuals graph.
      </Typography>
    );
  }
  ...
  if (nodes.length === 0)
    return (
      <Typography color="textSecondary">No mutuals to display.</Typography>
    );
  ...
  const handleNodeClick = async (id: string) => {
    ...
    if (!c && user) {
      try {
        const qc = getAppQueryClient();
        const single = await qc.ensureQueryData({
          queryKey: coreKeys.contactById(user.id, String(id)),
          queryFn: () => fetchContactById<ContactRow>(user.id, String(id)),
          staleTime: 60 * 60 * 1000,
        });
        if (single) c = single;
      } catch (err) {
    ...
    >
      ...
      >
        ...
        {nodes.map((n) => {
          ...
          const fill =
            n.depth === 0 ? "#1976d2" : n.depth === 1 ? "#4caf50" : "#ff9800";
          return (
            <g
              key={n.id}
            ...
            >
              ...
              <title>{n.name}</title>
              <text x={radius + 6} y={4} fontSize={12} fill="#222">
                {n.name}
              </text>
            </g>
          );
      ...
      <ContactDetailsDialog
        ...
        onRefresh={async () => {
          ...
          if (user) {
            ...
            // Keep behavior: ensure viewed contact is included even if not in list.
            if (
              contactId &&
              !list.find((c) => String(c.id) === String(contactId))
            ) {
              try {
                const single = await qc.fetchQuery({
                  queryKey: coreKeys.contactById(user.id, String(contactId)),
                  queryFn: () =>
                    fetchContactById<ContactRow>(user.id, String(contactId)),
                  staleTime: 0,
