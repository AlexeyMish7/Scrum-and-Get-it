  ...
  const submit = async () => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        interaction_quality:
          interactionQuality === "" ? null : Number(interactionQuality),
      };
      if (occurredAt) {
      ...
      if (res.error) {
        ...
        await queryClient.invalidateQueries({
          queryKey: coreKeys.contactInteractions(user.id, String(contactId)),
        });
        // Reset all fields
        setType("Note");
        setNotes("");
      ...
      <Stack spacing={2}>
        ...
        <Stack direction="row" spacing={2} flexWrap="wrap">
          <FormControlLabel
            control={
              <Checkbox
                checked={referralGenerated}
                onChange={(e) => setReferralGenerated(e.target.checked)}
              />
            }
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={jobOpportunityCreated}
                onChange={(e) => setJobOpportunityCreated(e.target.checked)}
              />
            }
