...
export default function ReferralRequest({ contactId, initialSelectedJob }: { contactId?: string | null; initialSelectedJob?: Record<string, any> | null }) {
	const { user, loading: authLoading } = useAuth();
	const [selectedJob, setSelectedJob] = useState<Job | null>(null);
	const [openAdd, setOpenAdd] = useState(false);
	const [suggestionsOpen, setSuggestionsOpen] = useState(false);
	const [suggestions, setSuggestions] = useState<Array<{ message: string; tone?: string; rationale?: string }>>([]);
			...
			(async () => {
				try {
					...
					if (!jr.error && jr.data) {
						...
						setSelectedJob({ id: jd.id != null ? String(jd.id) : String(maybeId), title: jd.job_title ?? jd.title ?? null, company: jd.company_name ?? jd.company ?? null });
						return;
					}
				} catch {
					// ignore
				...
				setSelectedJob({ id: maybeId != null ? String(maybeId) : undefined, title: j.job_title ?? j.title ?? null, company: j.company_name ?? j.company ?? null });
			})();
		} else {
			setSelectedJob({ id: maybeId != null ? String(maybeId) : undefined, title: j.job_title ?? j.title ?? null, company: j.company_name ?? j.company ?? null });
		}
	...
	async function fetchItems() {
		...
		setError(null);
		try {
			const res = await listReferralRequests(user.id, { eq: { contact_id: contactId }, order: { column: "created_at", ascending: false } });
			if (res.error) throw new Error(res.error.message || "Failed to load referrals");
			...
			if (missingJobIds.length && user) {
				await Promise.all(missingJobIds.map(async (jid) => {
					try {
						...
						if (!jr.error && jr.data) {
							const jd: any = jr.data;
							jobMap[jid] = { id: jd.id != null ? String(jd.id) : String(jid), title: jd.job_title ?? jd.title ?? null, company: jd.company_name ?? jd.company ?? null };
						}
					} catch {
						// ignore
					}
	...
	async function markCompleted(item: ReferralRow) {
		...
		try {
			...
			if (item.contact_id) {
				try {
					const contactRes = await getContact(user.id, String(item.contact_id));
					if (!contactRes.error && contactRes.data) {
						const existing = (contactRes.data as any).relationship_strength;
						const current = existing == null ? 0 : Number(existing) || 0;
						if (current < 10) {
							const next = Math.min(10, current + 2);
							const upd = await updateContact(user.id, String(item.contact_id), { relationship_strength: next });
							if (upd.error) {
								handleError(upd.error, "Failed to update relationship strength", "warning");
							} else {
								try {
									await confirm({
										title: "Congrats",
							...
							try {
								await confirm({
									title: "Info",
									message: "Relationship strength is already at the maximum value",
									confirmText: "Close",
								});
							} catch (err) {
								// ignore
	...
	async function saveEdit() {
		...
		try {
			...
			if ((editStatus ?? editing.status) === "Completed") payload.completed_at = new Date().toISOString();
	...
	return (
		...
		<Box>
			<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
				...
				<JobSearch onSelectJob={(job) => setSelectedJob(job)} selectedJob={selectedJob} showPreview={false} />
				...
				{(selectedJob || (initialSelectedJob as any)) && (
					<Typography variant="subtitle1" sx={{ fontWeight: 600 }}>
						{((selectedJob as any)?.job_title ?? selectedJob?.title ?? (initialSelectedJob as any)?.job_title ?? (initialSelectedJob as any)?.title ?? '')}
						{((selectedJob as any)?.company ?? (selectedJob as any)?.company_name ?? (initialSelectedJob as any)?.company ?? (initialSelectedJob as any)?.company_name) ? ` - ${((selectedJob as any)?.company ?? (selectedJob as any)?.company_name ?? (initialSelectedJob as any)?.company ?? (initialSelectedJob as any)?.company_name)}` : ''}
					</Typography>
				)}

				<Box sx={{ display: 'flex', gap: 1 }}>
							<Button variant="outlined" onClick={() => { setEditMessage(''); setOpenAdd(true); }} disabled={!selectedJob}>Add Referral Request</Button>
							<LoadingButton variant="outlined" loading={loading} onClick={async () => {
								...
								try {
									...
									const jobTitle = (selectedJob as any).job_title ?? (selectedJob as any).title ?? null;
									const jobCompany = (selectedJob as any).company_name ?? (selectedJob as any).company ?? null;
									const payload: Record<string, unknown> = {
										contact_id: contactId,
										job_title: jobTitle,
										job_company: jobCompany,
									};
									const resp = await aiClient.postJson<{ suggestions?: Array<{ message: string; tone?: string; rationale?: string }> }>('/api/generate/referral-request', payload, user.id);
									const s = (resp && resp.suggestions) || [];
									// Ensure each suggestion has a message string
									const normalized = s.map((it: any) => ({ message: String(it.message ?? it ?? ''), tone: it.tone, rationale: it.rationale }));
							...
							}} disabled={!selectedJob}>Generate Message & Preview</LoadingButton>
				</Box>
				...
				<List>
					{items.length === 0 && !loading && (
						<Typography color="textSecondary">No referral requests found.</Typography>
					)}
									...
									secondary={(
										<>
											...
											<Typography component="span" variant="caption" color="textSecondary">
												Requested: {formatDate(item.created_at ?? item.request_date)} • Completed: {formatDate(item.completed_at)}
											</Typography>
											{item.referral_notes && (
												<Typography variant="body2" sx={{ mt: 1 }}>{item.referral_notes}</Typography>
											)}
										</>
									)}
								/>
			...
			<Dialog open={openAdd} onClose={() => setOpenAdd(false)} fullWidth maxWidth="sm">
				...
				<DialogContent dividers>
					{selectedJob ? (
						<Typography variant="subtitle1" sx={{ fontWeight: 600 }}>{selectedJob.title ?? 'Job'} {selectedJob.company ? `- ${selectedJob.company}` : ''}</Typography>
					) : (
						<Typography color="textSecondary">No job selected.</Typography>
					)}

					<TextField label="Message to contact" fullWidth multiline minRows={3} sx={{ mt: 2 }} value={editMessage ?? ''} onChange={(e) => setEditMessage(e.target.value)} />

					<Typography variant="caption" color="textSecondary" sx={{ mt: 1, display: 'block' }}>Use "Generate & Preview" to get AI suggestions, then click "Use" to load one here. When ready, click "Add Referral Request" to save.</Typography>
				</DialogContent>
				<DialogActions>
					<Button onClick={() => setOpenAdd(false)}>Cancel</Button>
					<Button variant="contained" onClick={async () => {
						if (!contactId) { setError('Missing contact id'); return; }
						if (!selectedJob?.id) { setError('Select a job first'); return; }
						if (!user) { setError('Sign in required'); return; }
						setLoading(true);
						try {
							const payload: Record<string, unknown> = {
			...
			<Dialog open={suggestionsOpen} onClose={() => setSuggestionsOpen(false)} fullWidth maxWidth="md">
				...
				<DialogContent dividers>
					{suggestions.length === 0 && (
						<Typography color="textSecondary">No suggestions returned.</Typography>
					)}
					<List>
										...
										<>
											...
											{(s.tone || s.rationale) && (
												<Typography variant="caption" color="textSecondary" sx={{ display: 'block', mt: 1 }}>
													{s.tone ? `Tone: ${s.tone}` : ''}{s.tone && s.rationale ? ' • ' : ''}{s.rationale ?? ''}
												</Typography>
											)}
									</>
									)}
			...
			<Dialog open={Boolean(editing)} onClose={() => setEditing(null)} fullWidth maxWidth="sm">
				...
				<DialogContent dividers>
					<FormControl fullWidth sx={{ mt: 1 }}>
						...
						<Select labelId="ref-status-label" value={editStatus ?? 'Requested'} label="Status" onChange={(e) => setEditStatus(String(e.target.value))}>
					...
					<LocalizationProvider dateAdapter={AdapterDateFns}>
						<DatePicker
							...
							renderInput={(params: TextFieldProps) => <TextField {...params} fullWidth sx={{ mt: 2 }} />}
						/>
					</LocalizationProvider>

					<TextField sx={{ mt: 2 }} label="Notes" fullWidth multiline minRows={3} value={editMessage ?? ''} onChange={(e) => setEditMessage(e.target.value)} />
				</DialogContent>
				<DialogActions>
