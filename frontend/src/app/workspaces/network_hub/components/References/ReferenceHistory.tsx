...
import { coreKeys, fetchJobNotesByJobId } from "@shared/cache";
...
export default function ReferenceHistory({
  ...
  async function fetchReferences(contactId: string, userId: string) {
    ...
    setError(null);
    try {
      const res = await listReferences(userId, {
        eq: { contact_id: contactId },
        order: { column: "request_date", ascending: false },
      });
      if (res.error)
        throw new Error(res.error.message || "Failed to load references");
              ...
              if (!jr.error && jr.data) {
                ...
                try {
                  const notes = await queryClient.ensureQueryData({
                    queryKey: coreKeys.jobNotesByJobId(userId, Number(jid)),
                    queryFn: () =>
                  ...
                  if (Array.isArray(notes)) {
                    for (const note of notes) {
                      const appHist =
                        ...
                        note.applicationHistory ??
                        note.application_history_json ??
                        null;
                      if (Array.isArray(appHist)) {
                        for (const entry of appHist) {
                          ...
                          if (!dateCandidate) {
                            ...
                            const fromVal =
                              entry.from ?? entry.from_stage ?? null;
                            const looksLikeInterview =
                              (typeof toVal === "string" &&
                                toVal.toLowerCase() === "interview") ||
                              (typeof fromVal === "string" &&
                                fromVal.toLowerCase() === "interview");
                            if (looksLikeInterview) {
                              dateCandidate =
                                entry.changed_at ?? entry.changedAt ?? null;
                            }
                          }
                          ...
                          if (dateCandidate) {
                            ...
                            if (!Number.isNaN(d.getTime()))
                              interviewDates.push(d.toISOString());
                          }
                        }
                      }
                    }
      ...
      const withJobs = data.map((d) => {
        ...
        return {
          ...d,
          job: job ? { ...(job as any) } : job,
          successFromReference,
        };
      });
      setItems(withJobs);
    } catch (err: any) {
  ...
  async function saveEdit() {
    ...
    try {
      ...
      if (res.error)
        throw new Error(res.error.message || "Failed to update reference");
      setEditingItem(null);
      await fetchReferences(contactId!, user.id);
    } catch (e: any) {
    ...
    <Box>
      ...
      >
        ...
        <Button
          size="small"
          onClick={() =>
            contactId && user && fetchReferences(contactId, user.id)
          }
          disabled={loading}
        >
          Refresh
            ...
            {item.successFromReference && (
              ...
              >
                <Typography variant="body2" sx={{ fontWeight: 700 }}>
                  Success Interview Received After Reference
                </Typography>
              </Box>
            )}
                ...
                secondary={
                  <>
                    ...
                    <Typography
                      ...
                      color="textSecondary"
                    >
                      Requested: {formatDate(item.request_date)} • Target:{" "}
                      {formatDate(item.target_date)} • Completed:{" "}
                      {formatDate(item.completed_date)}
                      {item.job?.status
                        ? ` • Job Status: ${item.job.status}`
                        : null}
                    </Typography>
                    {item.notes && (
                  ...
                  <span>
                    ...
                    >
                      <CheckCircleOutlineIcon
                        color={
                          item.status === "Completed" ? "success" : "inherit"
                        }
                      />
                    </IconButton>
                  </span>
      ...
      >
        ...
        <DialogContent dividers>
          ...
          <TextField
            ...
            onChange={(e) => setEditTargetDate(e.target.value || null)}
          />
        ...
        <DialogActions>
          <Button onClick={() => setEditingItem(null)} disabled={loading}>
            Cancel
          </Button>
