...
const GenerateReferenceGuide: React.FC<Props> = ({ contact, job }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (Array.isArray(json?.talking_points))
        setTalkingPoints(json.talking_points.map((p: any) => String(p)));
      else if (Array.isArray(json?.data?.talking_points))
        setTalkingPoints(json.data.talking_points.map((p: any) => String(p)));
  ...
  return (
    <Box sx={{ mt: 2 }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
        Use AI to generate a short guide and talking points for your reference
        contact. The generation will use the selected job and your skills.
      </Typography>
