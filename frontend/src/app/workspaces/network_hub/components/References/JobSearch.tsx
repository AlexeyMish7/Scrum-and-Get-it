...
import { Box, TextField, Button, List, ListItem, ListItemButton, ListItemText, CircularProgress, Typography } from "@mui/material";
...
const JobSearch: React.FC<Props> = ({ onSelectJob, selectedJob, showPreview = true }) => {
    ...
    const load = async () => {
        ...
        setLoadingData(true);
        try {
            const res: Result<any[]> = await jobsService.listJobs(user.id);
            if (!res.error && res.data) setJobs(Array.isArray(res.data) ? res.data : [res.data]);
            else setJobs([]);
        } catch (err) {
    ...
    useEffect(() => { /* no-op by design */; /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [user, loading]);

    const filtered = jobs.filter((j) => {
        if (!query) return true;
        const q = query.toLowerCase();
        return (String(j.job_title ?? j.title ?? "").toLowerCase().includes(q) || String(j.company_name ?? "").toLowerCase().includes(q) || String(j.job_description ?? "").toLowerCase().includes(q));
    });
        ...
        <Box sx={{ mb: 2 }}>
            ...
            {showPreview && selectedJob && (
                <Box sx={{ mt: 1, p: 1, border: '1px solid', borderColor: 'divider', borderRadius: 1, bgcolor: 'background.paper' }}>
                    ...
                    <Typography variant="body2" sx={{ mt: 1 }}>{String((selectedJob as any).job_description ?? (selectedJob as any).description ?? '')}</Typography>
                </Box>
            )}
        </Box>
    );
...
const PaperList: React.FC<{ jobs: any[]; loading: boolean; onSelect?: (j: any) => void; selectedJob?: Record<string, unknown> | null }> = ({ jobs, loading, onSelect, selectedJob }) => {
    if (loading) return <Box display="flex" justifyContent="center" py={2}><CircularProgress /></Box>;
    if (!jobs.length) return <Typography color="text.secondary">No jobs found.</Typography>;
    return (
        <List sx={{ maxHeight: 240, overflow: 'auto' }}>
            {jobs.map((j) => (
                <ListItem key={j.id} disablePadding>
                    <ListItemButton selected={Boolean(selectedJob && selectedJob.id === j.id)} onClick={() => onSelect?.(j)}>
                        <ListItemText primary={j.job_title || j.title} secondary={j.company_name} />
                    </ListItemButton>
                </ListItem>
            ))}
        </List>
