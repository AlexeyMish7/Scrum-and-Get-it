...
export default function SalaryPrep() {
  ...
  async function generateMarket(roleIn: string, locIn: string) {
    ...
    const locality = /san francisco|sf|bay area|san jose|silicon valley/i.test(
      locIn
    )
      ? 1.4
      : /new york|nyc/i.test(locIn)
      ? 1.25
      : 1.0;
  ...
  async function generateTalkingPoints() {
    ...
    try {
      ...
      // AI generation failed, fall back to template suggestions
    }
    ...
    try {
      ...
      if (employment && employment.length) {
        ...
        for (const emp of employment) {
          if (emp.job_description && found < 3) {
            const txt = String(emp.job_description)
              .split(/\n+/)
              .map((s) => s.trim())
              .filter(Boolean)[0];
            if (txt) {
      ...
      if (profile && profile.bio && pts.length < 4) {
        pts.push(
          `Profile summary: ${String(profile.bio).slice(0, 220)}${
            String(profile.bio).length > 220 ? "..." : ""
          }`
        );
      }
      ...
      if (!pts.length)
        pts.push(
          `Highlight recent measurable impact (revenue, cost savings, efficiency) with specific numbers where possible.`
        );
      pts.push(
        `Reference market data: median for ${role || "this role"} in ${
          location || "your area"
        } is ${
          marketData ? `$${marketData.med.toLocaleString()}` : "market rate"
        }.`
      );
      pts.push(
      ...
      pts.push(
        `Frame the conversation around total compensation and provide clear alternatives.`
      );
    }
  ...
  function recordAccepted(id: string, accepted: number) {
    setOutcomes((cur) =>
      cur.map((o) => (o.id === id ? { ...o, acceptedSalary: accepted } : o))
    );
  }
      ...
      >
        ...
        <List>
          <ListItem disableGutters>
            <ListItemText
              primary="Base salary"
              secondary="Guaranteed annual base pay"
            />
          </ListItem>
          <ListItem disableGutters>
            <ListItemText
              primary="Bonus / performance"
              secondary="Target annual bonus or commission"
            />
          </ListItem>
          <ListItem disableGutters>
            <ListItemText
              primary="Equity / RSUs"
              secondary="Value, vesting schedule, and refresh expectations"
            />
          </ListItem>
          <ListItem disableGutters>
            <ListItemText
              primary="Benefits & perks"
              secondary="Healthcare, retirement match, PTO, relocation, signing bonus"
            />
          </ListItem>
        </List>
      </Box>
    ...
    <Box>
      ...
      <Stack spacing={2} sx={{ mb: 2 }}>
        ...
        <Stack direction="row" spacing={1}>
          <Button
            variant="contained"
            onClick={() => generateMarket(role, location)}
          >
            Get market data
          </Button>
          <Button variant="outlined" onClick={generateTalkingPoints}>
      ...
      <Stack direction="row" spacing={2} alignItems="flex-start">
        <Box sx={{ flex: 1 }}>
          <Paper sx={{ p: 2, mb: 2 }}>
            ...
            {marketData ? (
              <Box>
                <Typography>
                  Range: ${marketData.min.toLocaleString()} — $
                  {marketData.max.toLocaleString()}
                </Typography>
                <Typography>
                  Median: ${marketData.med.toLocaleString()}
                </Typography>
                <Box sx={{ mt: 1 }}>
                  <Typography variant="body2">
                    Where does your offer sit?
                  </Typography>
                  <Slider
                    value={offerPosition}
                    aria-label="offer-position"
                    min={0}
                    max={100}
                  />
                </Box>
              ...
              <Typography variant="body2" color="text.secondary">
                No market data yet — click Get market data.
              </Typography>
            )}
          </Paper>
          ...
          <Paper sx={{ p: 2, mb: 2 }}>
            ...
            {talkingPoints.length ? (
              ...
              <Typography variant="body2" color="text.secondary">
                Generate talking points to see suggestions.
              </Typography>
            )}
          </Paper>

          <Paper sx={{ p: 2, mb: 2 }}>{renderTotalCompFramework()}</Paper>
        </Box>
        ...
        <Box sx={{ width: 360 }}>
          <Paper sx={{ p: 2, mb: 2 }}>
            ...
            <Typography variant="body2" sx={{ mt: 1 }}>
              "Thank you — I’m excited about the role. Based on my research and
              recent contributions (X, Y), I’d like to discuss a base salary of
              ${targetSalary || "XXX,XXX"} or a structured increase in total
              comp. Could we explore options to bridge the gap?"
            </Typography>
            <Divider sx={{ my: 1 }} />
            ...
            <List>
              {outcomes.map((o) => (
                <ListItem key={o.id} disableGutters sx={{ py: 1 }}>
                  <ListItemText
                    primary={`${o.role} • ${o.location}`}
                    secondary={`Offered: ${
                      o.offeredSalary ? `$${o.offeredSalary}` : "N/A"
                    } • Target: ${
                      o.counterOffer ? `$${o.counterOffer}` : "N/A"
                    } • Accepted: ${
                      o.acceptedSalary ? `$${o.acceptedSalary}` : "-"
                    }`}
                  />
                  <Box sx={{ ml: 1 }}>
                    <Button
                      ...
                      onClick={() =>
                        recordAccepted(
                          o.id,
                          o.counterOffer || o.offeredSalary || 0
                        )
                      }
                    >
                      Mark accepted
