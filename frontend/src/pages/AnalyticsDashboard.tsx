...
const AnalyticsDashboard: React.FC = () => {
  ...
  const recommendations = useMemo(() => {
    ...
    if (count === 0) {
      recs.push(
        "Start logging your interviews to track progress and identify improvement areas."
      );
    } else if (conv < 0.1) {
      recs.push(
        "Work on tailoring your applications and improving your initial screening responses."
      );
    } else if (conv < 0.2) {
      recs.push(
        "Practice mock interviews focused on common formats to boost your confidence."
      );
    } else if (conv >= 0.3) {
      recs.push(
        "Great conversion rate! Continue refining your approach and aim for consistency."
      );
    }
    ...
    if (count < 5) {
      recs.push(
        "Schedule regular mock sessions to accelerate improvement and build confidence."
      );
    } else if (mockAvg !== null && realAvg !== null) {
      if (improvement && improvement > 10) {
        recs.push(
          `Excellent progress! Your real interviews score ${Math.round(
            improvement
          )} points higher than mocks.`
        );
      } else if (improvement && improvement < -5) {
        recs.push(
          "Real interviews are scoring lower than mocks. Focus on managing interview anxiety."
        );
      }
    } else if (realAvg !== null && mockAvg === null) {
      recs.push(
        "Consider doing mock interviews to practice in a low-pressure environment before real ones."
      );
    }

    // Format-specific recommendations
    const weakestFormat = formatBreakdown.sort(
      (a, b) => a.conversion - b.conversion
    )[0];
    if (weakestFormat && weakestFormat.conversion < conv - 0.1) {
      recs.push(
        `Focus on improving ${weakestFormat.format} interviews - your conversion rate is lower in this format.`
      );
    }
    ...
    if (topThemes.length > 0) {
      ...
      if (mostCommon.count >= 3) {
        recs.push(
          `🎯 Critical: "${mostCommon.theme}" appears in ${mostCommon.count} interviews. This is your #1 improvement priority.`
        );
      } else {
        recs.push(
          `Common feedback areas: ${themeNames}. Create action items to address these.`
        );
      }
    }
    ...
    if (count >= 5 && conv < 0.15) {
      recs.push(
        "Review your interview recordings or notes to identify patterns in unsuccessful interviews."
      );
    }
    ...
    <Box sx={{ p: { xs: 2, md: 4 } }}>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          <Tooltip title="Back to Interview Hub">
            <IconButton onClick={() => navigate("/interviews")} size="large">
              <ArrowBackIcon />
            </IconButton>
          </Tooltip>
          <Box>
      ...
      >
        <Box>
          <SummaryCard
            ...
            value={
              overview
                ? `${Math.round((overview.conversionRate ?? 0) * 100)}%`
                : "—"
            }
          />
          <Typography
        ...
        {confidenceData.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={160}>
            ...
            >
              ...
              <XAxis dataKey="date" />
              <YAxis domain={[0, 10]} />
              <RechartsTooltip formatter={(v: any) => `${v}/10`} />
              <Line
                type="monotone"
                dataKey="confidence"
                stroke="#ff9800"
      ...
      >
        <Paper variant="outlined" sx={{ p: 2 }}>
          ...
          <Box sx={{ mt: 1 }}>
            {(trends?.industryComparison ?? [])
              .filter((i: any) => i.industry?.trim())
              .map((i: any) => (
                <Box
                  key={i.industry}
                  sx={{
        ...
        <Paper variant="outlined" sx={{ p: 2 }}>
          ...
          <Box sx={{ mt: 1, display: "flex", gap: 1, flexWrap: "wrap" }}>
            ...
            {(overview?.feedbackThemes ?? []).length === 0 && (
              <Typography color="text.secondary">
                No feedback themes yet. Add feedback to your interviews to track
                improvement areas.
              </Typography>
            )}
          </Box>
      ...
      <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
        ...
        <Box sx={{ mt: 1 }}>
          {(trends?.cultureComparison ?? [])
            .filter((c: any) => c.culture?.trim())
            .map((c: any) => (
              <Box
                key={c.culture}
                sx={{
              ...
              >
                <Typography sx={{ textTransform: "capitalize" }}>
                  {c.culture.trim().replace(/-/g, " ")}
                </Typography>
                <Typography fontWeight="medium">
                  {Math.round((c.conversion ?? 0) * 100)}%
                </Typography>
              </Box>
            ))}
          {(trends?.cultureComparison ?? []).length === 0 && (
            <Typography color="text.secondary">
              Add company culture to your interviews
            </Typography>
          )}
        </Box>
      </Paper>
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 3 }}>
        <SummaryCard
          ...
          value={
            overview
              ? `${
                  overview.mockVsReal?.mockAverage
                    ? Math.round(overview.mockVsReal.mockAverage)
                    : "—"
                } / ${
                  overview.mockVsReal?.realAverage
                    ? Math.round(overview.mockVsReal.realAverage)
                    : "—"
                }`
              : "—"
          }
        ...
        <SummaryCard
          title="Improvement"
          value={
            overview && overview.mockVsReal?.improvement
              ? `${overview.mockVsReal.improvement > 0 ? "+" : ""}${Math.round(
                  overview.mockVsReal.improvement
                )}`
              : "—"
          }
        />
        ...
        <Box sx={{ mt: 1 }}>
          {overview && overview.conversionRate > 0.25 && (
            <Typography variant="body2" sx={{ mb: 1 }}>
              ✓ Your {Math.round((overview.conversionRate ?? 0) * 100)}%
              conversion rate is above the typical 15-20% range!
            </Typography>
          )}
          {overview && overview.interviewsCount >= 10 && (
            <Typography variant="body2" sx={{ mb: 1 }}>
              ✓ You've completed {overview.interviewsCount} interviews -
              consistent practice leads to mastery.
            </Typography>
          )}
          {overview &&
            overview.mockVsReal?.improvement &&
            overview.mockVsReal.improvement > 5 && (
              <Typography variant="body2" sx={{ mb: 1 }}>
                ✓ {Math.round(overview.mockVsReal.improvement)} point
                improvement shows your practice is paying off!
              </Typography>
            )}
          {(!overview || overview.interviewsCount < 5) && (
            <Typography variant="body2" color="text.secondary">
              Complete more interviews to unlock personalized benchmarking
              insights.
            </Typography>
          )}
        </Box>
